...
ERROR_MESSAGES = {"bad_username_or_password": "", "bad_captcha": "32 Invalid code"}
...
def camp_course(course_code):
    my_course_registration_elem = driver.find_element(
        "link text", "My Course Registration"
    )
    ...
    while True:
        ...
        availabilities = driver.find_elements_by_xpath(
            "//div/table/tbody/tr[position() mod 3 = 2]/td[12]"
        )
        for availability in availabilities:
            if availability.text != "0":
                notification.notify(
                    title="NEW AVAILABILITY!",
                    message="New availability for course {}".format(course_code),
                )
                sys.exit(0)
...
if __name__ == "__main__":
    login(username="2001461", password="sohminxuan01110687902!")
    camp_course("MPU32193")
